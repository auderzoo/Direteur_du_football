...
// ---- Attributs détaillés (rapport de scouting, échelle 1-20) ----
function generateAttributes(player) {
  ...
  return {
    vitesse:   attr(pos === "ATT" ? 2 : pos === "DEF" ? 0 : pos === "GB" ? -3 : 1),
    tir:       attr(pos === "ATT" ? 3 : pos === "MIL" ? 1 : pos === "GB" ? -6 : -3),
    passe:     attr(pos === "MIL" ? 3 : pos === "GB" ? -4 : pos === "DEF" ? 0 : 1),
    defense:   attr(pos === "DEF" ? 3 : pos === "GB" ? 2 : pos === "MIL" ? 0 : -4),
...
function pickScorers(club, goals, dateKey) {
  ...
  for (let i = 0; i < goals; i++) {
    ...
    if (scorer) scorers.push(`${scorer.firstName} ${scorer.lastName}`);
  }
  return scorers;
}
...
function runMonthlyFinanceTick() {
  allClubs().forEach(club => {
    ...
    const ticketing = Math.round((club.homeMatchesThisMonth || 0) * club.reputation * 1800);
    ...
    const otherIncome = club.monthlyOtherIncome || 0;
    ...
    club.financeHistory.unshift({
      month: monthKey(state.date), sponsor, ticketing, merchandising, otherIncome, wages, transfersNet,
...
function migrateSave() {
  ...
  allClubs().forEach(c => {
    c.players.forEach(p => {
      ...
      if (!p.attributes) p.attributes = generateAttributes(p);
    });
  });
}
