...
function seasonRollover() {
  Object.values(state.countries).forEach(country => {
    ...
    const humanClubsHere = [...country.leagues.d1.clubs, ...country.leagues.d2.clubs].filter(c => isUserClub(c.id));
    ...
    if (d1sorted.length >= 4 && d2sorted.length >= 3) {
      ...
      pairs.forEach(([teamD1, teamD2]) => {
        ...
        state.news.unshift({ date: fmtDate(state.date), text });
        if (isUserClub(teamD1.id) || isUserClub(teamD2.id)) state.log.unshift({ date: fmtDate(state.date), text });
  ...
  allClubs().forEach(club => {
    ...
    // centre de formation : 1 à 2 jeunes joueurs générés chaque saison pour compenser les départs
    const youthCount = randInt(1, 2);
    for (let i = 0; i < youthCount; i++) {
      ...
      youth.overall = clamp(youth.overall - randInt(8, 18), 30, 90);
      youth.potential = clamp(youth.potential, youth.overall, 94);
      youth.contractEndYear = state.season.year + randInt(2, 4);
      ...
      league.fixtures.forEach(round => {
        ...
        round.matches.forEach(m => {
          ...
          if (isUserClub(home.id) || isUserClub(away.id)) {
            recordH2H(home.id, away.id, homeGoals, awayGoals, league === country.leagues.d1 ? "Élite" : "Nationale");
