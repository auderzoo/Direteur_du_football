      ...
      <tbody>
        ${rows.map(p => {
          ...
          <tr class="${injured ? "injured-row" : ""}">
            <td>${p.firstName} ${p.lastName}${p.releaseClause ? " 🔓" : ""}</td>
            <td>${p.position}</td>
            <td>${p.age}</td>
            <td>${p.overall}</td>
            <td>${p.potential}</td>
...
function renderFinance() {
  ...
  return `
    ...
    <table class="squad-table">
      <thead><tr><th>Mois</th><th>Sponsoring</th><th>Billetterie</th><th>Merchandising</th><th>Autres (coupes)</th><th>Salaires</th><th>Transferts (net)</th><th>Solde</th><th>Trésorerie après</th></tr></thead>
      <tbody>
        ${club.financeHistory.map(h => `
          <tr>
            ...
            <td>${fmtMoney(h.otherIncome || 0)}</td>
...
function attrLabel(key) {
  return { vitesse: "Vitesse", tir: "Tir", passe: "Passe", defense: "Défense", physique: "Physique", technique: "Technique" }[key] || key;
}
